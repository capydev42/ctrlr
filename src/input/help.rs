...
const META: &[(A, &str, &str, &str)] = &[
    ...
    (A::EditCommand, "Edit Command", "Edit the command inline before running it", "Actions"),
    (A::OpenExternalEditor, "Edit in $EDITOR", "Open the command in $VISUAL/$EDITOR, then run it", "Actions"),
